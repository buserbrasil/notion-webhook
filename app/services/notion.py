...
class NotionService:
    """Service for interacting with Notion API and validating webhooks."""
    ...
    async def validate_webhook_signature(request: Request, body: bytes) -> bool:
        ...
        signature_header = request.headers.get(NotionService.SIGNATURE_HEADER)
        if not signature_header:
            logger.warning("Missing %s header", NotionService.SIGNATURE_HEADER)
            return False
    ...
    async def fetch_entity_data(
        ...
        Args:
            entity_id: The ID of the Notion entity (page, database, etc.)
            entity_type: The type of entity (page, database, etc.)

        Returns:
            dict: The entity data from Notion
        ...
        # For pages, we want to include the content by default
        params = {}
        if entity_type == "page":
            # To get page content, we'd normally need a separate request for blocks
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                ...
                if entity_type == "page":
                    blocks = await NotionService._fetch_block_children(
                        client, entity_id, headers
                    )
                    entity["blocks"] = blocks
                    entity["markdown"] = NotionService.blocks_to_markdown(blocks)
                    entity["title"] = NotionService.extract_title(entity, "page")
                elif entity_type == "database":
                    entity.setdefault("markdown", "")
                    entity["title"] = NotionService.extract_title(entity, "database")

                if entity_type == "page" and "url" not in entity:
                    entity["url"] = NotionService.build_page_url(entity)
        ...
        while True:
            ...
            for block in payload.get("results", []):
                if block.get("has_children"):
                    block["children"] = await NotionService._fetch_block_children(
    ...
    def blocks_to_markdown(blocks: List[Dict[str, Any]], indent_level: int = 0) -> str:
        ...
        for block in blocks:
            ...
            text = NotionService._rich_text_array_to_markdown(
            ...
            if block_type == "paragraph":
                ...
                if block_type == "to_do":
                    ...
                    image_url = NotionService._extract_file_url(content)
                    if image_url:
                        line = f"![Image]({image_url})"
                elif block_type == "bookmark":
                    url = content.get("url", "")
                    caption = NotionService._rich_text_array_to_markdown(
                        content.get("caption", [])
                    )
            ...
            if not children and content.get("children"):
                children = content.get("children")

            if children:
                child_markdown = NotionService.blocks_to_markdown(
                    children,
    ...
    def _rich_text_array_to_markdown(rich_text_array: List[Dict[str, Any]]) -> str:
        return "".join(
            NotionService._rich_text_to_markdown(segment) for segment in rich_text_array
        )
    ...
    def build_page_url(page: Dict[str, Any]) -> Optional[str]:
        ...
        if isinstance(title, dict):
            ...
            slug = NotionService._rich_text_array_to_markdown(rich_text)
        ...
        return f"{base_url}{sanitized_slug}-{without_hyphen}" if sanitized_slug else f"{base_url}{without_hyphen}"
    ...
    async def save_entity(
        ...
        """
        Save entity data to the database using the appropriate adapter.

        Args:
            entity_id: The ID of the entity
            entity_type: The type of entity ("page", "database", etc.)
            entity_data: The data from the Notion API

        Returns:
            Optional[bool]: True when persisted, False when the write fails,
            None when persistence is skipped.
        """
        if not settings.has_database_credentials:
            logger.debug(
        ...
        if not title:
            title = NotionService.extract_title(entity_data, entity_type)
